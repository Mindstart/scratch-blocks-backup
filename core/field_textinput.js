...
Blockly.FieldTextInput = function(text, opt_validator, opt_restrictor) {
    var restrictor = this.getRestrictor();
...
Blockly.FieldTextInput.prototype.setText = function(newText) {
    ...
    if (newText.size > this.maxInputLength) {
        newText = newText.substring(0, this.maxInputLength)[0];
    }
    if (this.hasInvalidText(newText)) {
        return "";
    }
    newText = '"' + newText + '"';
...
/**
 * check whether text contains invalid characters
 * @param {string} text True if checked.
 */
Blockly.FieldTextInput.prototype.hasInvalidText = function(text) {
    // var testInput = /^[A-Za-z0-9]*$/.test(newText);
    // var testInput = /[^\x00-\x7F]+/.test(newText);
    // var testInput = /[^\u0000-\u007F]+/.test(newText);
